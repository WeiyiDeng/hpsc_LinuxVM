# w: run from command line starting with sudo !!

import mysecrets                        # w: my own module for safely storing passwords
...
def requestSuggestions(band_name):  
    ...
    google_password = mysecrets.pswgen(google_username)
    mes=0
...
def requestBands(search_term):  
    ...
    trend_payload_lab = {'q': search_term, 'cat': '35'}
    ...
    while mes==0:
        ...
        # except Exception:
	except (pytrends.request.RateLimitError, pytrends.request.ResponseError, IndexError):
            renew_tor()
            connectTor()
            showmyip() #optional
            pytrend = TrendReq(google_username, google_password, custom_useragent=randomName()) #connect to Google
...
f = open('/home/uwhpsc/wdocs/pytrends/bands4061_cat35/search_list.csv', "w")
f2 = open('/home/uwhpsc/wdocs/pytrends/bands4061_cat35/problem_list.csv', "w")
f3 = open('/home/uwhpsc/wdocs/pytrends/bands4061_cat35/stopped_list.csv', "w")
...
for band in search_terms:
	try:
		...
		path = "/home/uwhpsc/wdocs/pytrends/bands4061_cat35/" + str(index+1) + "_cat35.csv"
		...
		print([band + '_stopped'])
		mydoc3.writerow([[search_list[index]],[index+1]])
	finally:
...
# credit: https://github.com/jiby91/Big_data_trading_algorithm/blob/master/Pytrends.ipynb
